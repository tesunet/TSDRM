...
class ServerByPara(object):
    ...
    def exec_linux_cmd(self, succeedtext, port=22):
        ...
        try:
            self.client.connect(hostname=self.host, username=self.user, password=self.pwd, timeout=10, port=port)
        # except (socket.timeout, paramiko.ssh_exception.SSHException, paramiko.ssh_exception.NoValidConnectionsError) as e:
            ...
            return {
                "exec_tag": 1,
                "data": "连接服务器({0})失败 {1}".format(self.host, e),
                "log": "连接服务器({0})失败 {1}".format(self.host, e),
            }
        try:
...
if __name__ == '__main__':
    ...
    server_obj = ServerByPara(r"ls", "192.168.1.152", "root", "Tesunet@2020", "Linux")
